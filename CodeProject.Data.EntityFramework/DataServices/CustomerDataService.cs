...
namespace CodeProject.Data.EntityFramework
{
    ...
    /// </summary>
    public class CustomerDataService : EntityFrameworkService, CodeProject.Interfaces.ICustomerDataService
    {
        ...
        {
            //List<int> cids = customerIds.ToString().Split(',').Select(int.Parse).ToList();
            var selectedCustomers = dbConnection.Customers.Where(f => customerIds.Contains(f.CustomerID)).ToList();
            //selectedCustomers.ForEach(dbConnection.Customers.DeleteAllOnSubmit()
           // dbConnection.Customers.RemoveRange(selectedCustomers);
